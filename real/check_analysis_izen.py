...
if __name__ == "__main__": 
    ...
    # data_path = main_path / "Data" / "tomomu" / "local_mimosa" / "DATA" / "Izen" / "COLIS_2274" / "MID" / "1_analyse"

    irun, iana= [int(arg) for arg in sys.argv[1:3]]
    
    str_run = f"run{irun}" # 
    data_path = main_path / "Data" / "izen" / str_run
    ...
    fana = data_path / filename
    # fana = data_path / filename
    print(f"Load file {fana}")

    dout = data_path / 'out' / f"{str_run}" / f"{str_ana}"
    dout.mkdir(parents = True, exist_ok = True)
    
    prefix = 'MGv3'
 
    # ltime_var = ['time', 'deltat']
    lclus_var = [f'{prefix}_ClusPos', f'{prefix}_ClusAmpl', f'{prefix}_ClusMaxStripAmpl', f'{prefix}_ClusSize', f'{prefix}_ClusTOT',  f'{prefix}_ClusT'] #f'{prefix}_ClusMaxSample', f'{prefix}_StripMaxAmpl', 
    ...
    bins[f'{prefix}_ClusMaxSample'] = 10
    ...
    vb_range, vt_range, tel_range = range(0, 8), range(8, 16), range(16, ndet_tot)
    dict_range_det = {'VB': vb_range, 'VT': vt_range, 'TEL':tel_range, }
    ...
    vb, vt = IZEN.veto_plane
    tel = IZEN.telescope[0]

    l_ens = [vb, vt, tel]

    for iran,  ens in enumerate(l_ens): 
        ...
        for ivar, (key, var) in enumerate(dict_det_clus_var.items()):
            ...
            for idet in range(ndet) : 
                ...
                if ivar == 0 : 
                    ...
                    ax.set_ylabel(f'{coord}{iran*len(det_range)+idet}')
        ...
        for ipan in range(npan):
            ...
            sdet = f"X{iran*len(det_range)+2*ipan}/Y{iran*len(det_range)+2*ipan+1}"
