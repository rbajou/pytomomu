...
ec.detector_ensemble.plot3D(ax=ax, **kwargs)
...
cbar = fig.colorbar(ScalarMappable(norm=norm, cmap=cmap), ax=ax, pad=0.1, shrink=0.66)
cbar.set_label('Amplitude [ADC]')

xyz_tel = ec.xyz[:, -4:, :]
mask_3p = np.count_nonzero(np.all(xyz_tel!=-999, axis=-1), axis=-1)
mask_3p = np.count_nonzero(mask_3p, axis=-1) > 2
...
for i, iev in enumerate(riev) : 
    ...
    if mask_3p[iev] :
        ...
        ax.plot(xyz_trk[:,0],xyz_trk[:,1], xyz_trk[:,-1],
            c=col_riev[i], linewidth=0.75)
        fontdict= {"size":"xx-large", "color": col_riev[i], "weight":"bold", "ha":"center"}
        ax.text(xyz_trk[-1,0],xyz_trk[-1,1], xyz_trk[-1,-1], s=iev, fontdict=fontdict)
        ax.text(xyz_trk[0,0],xyz_trk[0,1], xyz_trk[0,-1], s=iev, fontdict=fontdict)
...
for s in [5, 10, 20]:
    ...
    leg_hdl.append(hdl)
ax.legend(title='Cluster size', handles = leg_hdl, loc='upper left', bbox_to_anchor=(0.1,1))

det_side = tel.detectors[0].side
ax.set_xlim(-det_side, det_side)
ax.set_ylim(-det_side, det_side)
ax.set_zlim(zmin-100, zmax+100)
ax.set_title(f"Event(s) {', '.join([str(i) for i in np.sort(riev)])}", weight='bold', )
if CURRENT_SURVEY.name=="izen":     ax.set_zticks([])
plt.tight_layout()

dout = rout / "evd"
dout.mkdir(parents=True, exist_ok=True)
fout = dout / f"evt3D_{str_riev}.png"
fig.savefig(fout)
