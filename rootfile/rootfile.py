...
class RootFile: 
    ...
    def open(self,  treename:str="T", **kwargs): 
        if isinstance(self.file, Path) : self.file = str(self.file)
        self.rootfile = uproot.open(self.file + f':{treename}', **kwargs)
        self._branch_names = list(self.rootfile.keys())

    def get_content(self, n_events:int, branch_list:list=None,start_event:int=0, 
                     step_size:int=50000, library:int='np'): #, save_npy=False, save_path=None, save_name=None):
        '''
        ...
        index = 0
        iterator = rf.iterate(step_size=step_size, 
                          filter_name=branch_list, 
                          entry_start=start_event, 
                          entry_stop=min(start_event+n_events, n_events_tot), 
                          library=library)
        i = 0 
        while True:
            try:
                ...
                print(f"Error in batch {i}; for entries in [{index}, {index+batch_size}]")
